...
class Path(private val _file: File) {
	...
	def subfolders: Array[Path] =
		file.listFiles.filter(_.isDirectory).map(new Path(_))
	def listFiles(fileNameRegExp: String): Array[Path] = {
		if(isDir)
			(new FileSearch()).listFiles(file.getPath,fileNameRegExp).map(new Path(_))
		else
			Array.empty
	}
}
...
object Path{
  def apply(file: File) = {
	...
	def escapePath(str: String): String = str.replace(":","-")
}
